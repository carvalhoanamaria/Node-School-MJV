...
router.post('/', (req: Request, res: Response) => {
    if(req.body.description == ''){
       return res.status(400).send({message: 'Não foi possivel realizar o cadastro do produto pois não tem uma descrição.'});
    }
...
router.put('/:pid', (req: Request, res: Response) => {
    const productIndex = products.findIndex((student) => student.id === Number(req.params.pid));
    if(productIndex === -1){
...
router.delete('/remove/:pid', (req: Request, res: Response) => {

   const productIndex = products.findIndex((product) => product.id === Number(req.params.pid));
